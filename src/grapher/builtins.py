...
def random_hex_color():
	color = hex(random.randrange(0, 16777215, 1))[2:]
	while len(color) != 6:
		color = '0' + color
	return '#' + color
...
def extract_named_values(nv_list, calling_identifier):
	if not nv_list.type == types.LIST:
		raise exceptions.IncorrectType(calling_identifier, expected_types=types.LIST)
	pv = []
	for i, nv in enumerate(nv_list.value):
